...
const LISTS = {
  GENERAL: 3,           // Test Landing General — todos los contactos caen aquí siempre
  PROFESIONAL: 4,        // Profesional Sensible
  CONSCIENTE: 5,         // Mujer Consciente (perfil "La Mente Consciente")
  BUSCADOR: 6            // Buscador Masculino
};

// Mapea el resultado del test al ID de lista correspondiente.
// Se usa una combinación de nombre de perfil y producto recomendado, porque
// los 5 tests devuelven nombres de perfil distintos pero los productos son siempre los mismos.
function getListIdForProfile(profileName, producto) {
  const name = (profileName || "").toLowerCase();
  const prod = (producto || "").toLowerCase();

  // Perfiles que apuntan claramente al nicho masculino / saboteador
  ...
  // Perfiles que apuntan al nicho de la mente consciente / trabajo interior profundo
  ...
  // Perfiles que apuntan al nicho de saturación / profesional sensible
  ...
  // Si el nombre del perfil no es suficiente, usamos el producto recomendado como respaldo
  ...
  return null; // Si nada coincide, el contacto solo cae en la lista general
}
...
exports.handler = async function (event) {
  ...
  // RGPD: si la persona no marcó el consentimiento explícito, no se guarda
  // (el frontend solo debe llamar a esta función cuando optIn === true)
  if (optIn !== true) {
    return { statusCode: 400, body: JSON.stringify({ error: "Falta consentimiento RGPD" }) };
  }

  // Construir la lista de IDs a las que se debe añadir el contacto
  ...
  const payload = {
    ...
    listIds: listIds,
    updateEnabled: true // si el contacto ya existe, actualiza sus datos en lugar de fallar
  };
  ...
  try {
    ...
    // Brevo devuelve 204 en éxito de update, 201 en creación
